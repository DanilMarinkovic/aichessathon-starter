...
def _budget_ms(time_left_ms: int) -> float:
    ...
    This variant is that measurement: the divisor is 14. Eight further rated games confirm the
    same shape independently -- 4.3s a move over moves 1-15 decaying to 0.7s after move 60, in
    games that ran 46 to 162 moves and were decided late every time. More thinking time is not
    automatically more Elo, though: on the position that lost round 14, twenty times the search
    picked the same losing move. So it is a clock match that decides, not this reasoning.
    """
    usable = max(1.0, time_left_ms - OVERHEAD_MS)
    increment = max(0.0, _increment_ms)
    budget = usable / 14.0 + increment * 0.6
