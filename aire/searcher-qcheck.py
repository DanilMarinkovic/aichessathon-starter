...
def quiescence(
    ...
    Being in check is a separate case, and this function used to miss it entirely. The side to
    move cannot pass, so there is no stand-pat to fall back on and no reason the static score
    bounds anything; captures alone leave out the king moves and blocks that are usually the
    whole reply; and a node with no capture-evasion returned a quiet evaluation for what may
    be mate. Every capture that gives check lands on such a node, so all three applied at once.
    The main search never arrives here in check because it extends instead, which is why this
    stayed hidden. Strong engines give quiescence a dedicated evasion path; this is that path.
    """
    control[NODES] += 1
    ...
    if checked != 0:
        # best starts at the mate score, so a node where no move turns out to be legal returns
        # mate rather than a static number. generate(..., 0) is every move, not just captures.
        stand_pat = np.int32(-MATE + ply)
        best = stand_pat
    ...
    for index in range(count):
        ...
        if (
            checked == 0
            and move_promotion(move) == 0
            and see_ge(states[ply], move, np.int64(0)) == 0
        ):
            continue
